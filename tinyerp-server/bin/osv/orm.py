		...
		if len(fields_pre) :
			...
			if d1:
				...
				if not cr.rowcount == len({}.fromkeys(ids)):
					raise except_orm("Error d'Accés", "Esteu intentant saltar-vos una regla d'accés,\npotser esteu a una altra empresa?\n(Tipus de document: %s)." % self._description)
			else:
...
#					record[f]=False
	...
	def unlink(self, cr, uid, ids, context=None):
		...
		if delta and self._log_access:
			...
			res= cr.fetchone()
			if res and res[0]:
				raise except_orm('Error de Concurrència', 'Aquest registre ha estat modificat mentre el tenies en pantalla.\nRefresca la pantalla abans de continuar.')
	...
	def write(self, cr, user, ids, vals, context=None):
		...
		if len(upd0):
			...
			if d1:
				...
				if not cr.rowcount == len({}.fromkeys(ids)):
					raise except_orm("Error d'Accés", "Esteu intentant sgravar un registre que no existeix.\n(Tipus de document: %s)." % self._description)
