...
"""
  ...
  Carrega factures a partir del diari de compres
...
def get_partner(cr,uid,supp):
    pool = pooler.get_pool(cr.dbname)
    #cr.execute("select id from res_partner_supplier where name ='%s' " % supp)
    #print cr.fetchall()
    ids = pool.get("res.partner.supplier").search(cr,uid,[('name','=',supp)])
    if ids:
        c = pool.get("res.partner.supplier").browse(cr,uid,ids[0])
        return c.partner_id
    else:
        return False
...
class wizard_load_invoice(wizard.interface):
    ...
    def _load(self, cr, uid, data, context):
        ...
        tax["TL"]={'4':3,'7':2,'16':1}
        tax["TJ"]={'4':7,'7':6,'16':5}
        tax["TC"]={'4':11,'7':10,'16':9}
        tax["BD"]={'4':15,'7':14,'16':13}
        ...
        for lin in f:
            if lin.startswith('^IVA('):
                ...
                #fac['date_due']
                partner = get_partner(cr,uid,supp)
                ...
                #fac['move_id'] 
                fac['currency_id'] = 1
                fac['journal_id'] = get_journal(cr,uid,"COMP",company_id)
                ...
                #    #amount= -float(amount)
                if por_iva == "16" and not por_iva2 and not por_iva3 and not amount2 and not amount3:
                    continue
                ...
                if not ids:
                    print year,cname,inv,"<",supp,">","NO TROBADA"
                    continue

                #print year,cname,inv,"<",supp,">",por_iva,por_iva2,por_iva3,amount,amount2,amount3
                print year,cname,inv,"<",supp,">",por_iva,por_iva2,por_iva3
                """
                if ids:
                ...
                v_acc_id = get_account(cr,uid,'60002',company_id)
                ...
                if por_iva == '0':
                    iline['invoice_line_tax_id'] = [(6,0,[])]
                else:
                    #print tax[cname]
                    #print tax[cname][por_iva]
                    iline['invoice_line_tax_id'] = [(6,0,[tax[cname][por_iva]])]
                s = [('name','=',iline['name']),('invoice_id','=',ids[0])]
                ...
                if por_iva2:
                    ...
                    iline['invoice_id'] = ids[0]
                    iline['account_id'] = v_acc_id
                    iline['price_unit'] = amount2
                    iline['quantity'] = 1
                    iline['discount'] = 0.0
                    #tax_id = pool.get('account.account').read(cr,uid,[v_acc_id],['tax_ids'])[0]
                    #iline['invoice_line_tax_id'] = [ (6,0,tax_id['tax_ids']) ]
                    iline['invoice_line_tax_id'] = [ (6,0,[tax[cname][por_iva2]]) ]
                    s = [('name','=',iline['name']),('invoice_id','=',ids[0])]
                    l_ids = pool.get("account.invoice.line").search(cr,uid,s)
                    if l_ids:
                        pool.get("account.invoice.line").write(cr,uid,l_ids,iline)
                ...
                if por_iva3:
                    ...
                    iline['invoice_id'] = ids[0]
                    iline['account_id'] = v_acc_id
                    iline['price_unit'] = amount3
                    iline['quantity'] = 1
                    iline['discount'] = 0.0
                    #tax_id = pool.get('account.account').read(cr,uid,[v_acc_id],['tax_ids'])[0]
                    #iline['invoice_line_tax_id'] = [ (6,0,tax_id['tax_ids']) ]
                    iline['invoice_line_tax_id'] = [ (6,0,[tax[cname][por_iva3]]) ]
                    s = [('name','=',iline['name']),('invoice_id','=',ids[0])]
                    l_ids = pool.get("account.invoice.line").search(cr,uid,s)
                    if l_ids:
                        pool.get("account.invoice.line").write(cr,uid,l_ids,iline)
...
wizard_load_invoice('carreras.load_in_invoice')
