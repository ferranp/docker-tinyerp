...
"""
  ...
  Carrega de clients i proveidors

    ****** AQUEST PROGRAMA ES OBSOLET !!!!!! ******
    
    Ara hi ha load_customer per clients i load_supplier per proveidors
...
class wizard_load_partner(wizard.interface):
    ...
    def _load(self, cr, uid, data, context):
        ...
        for lin in f:
            ...
            if lin.startswith('^CLI('):
                ...
                print "%2s %4s %10s %s" % (cname,snumcli,ref,name)
                ...
                if d:
                    ...
                    new_ult_fac =  False
                    if len(lin0[13]) > 7:
                        new_ult_fac = lin0[13][0:3] + '-' + lin0[13][4:5] + '-' + lin0[13][7:8]
                ...
                cust['invoice_copies']= lin3[0] and int(lin3[0])+1 or 1
...
##                if tarifa:
##                    sql = "update pricelist_partner set partner_id=%d where company_id=%d and name like '%s' " % (emp_id,company_id,tarifa + '%')
##                    #print sql
##                    cr.execute(sql)
##                tar_id = pool.get('product.pricelist').search(cr,uid,[('name','=',tarifa)])
##                #print "<%s>,%s " %(tarifa,str(tar_id))
##                if tar_id:
##                    print tarifa
##                    tar_id = tar_id[0]
##                    res_id = 'res.partner,%d' % emp_id
##                    
##                    d = pool.get('ir.property').search(cr,uid,[('name','=','property_product_pricelist'),('res_id','=',res_id)])
##                    p = {}
##                    p['name']='property_product_pricelist'
##                    p['res_id']= res_id
##                    p['company_id'] = False
##                    p['value']= 'product.pricelist,%d' % tar_id
##                    p['fields_id']= pool.get('ir.model.fields').search(cr,uid,[('name','=','property_product_pricelist')])[0]
##                    
##                    if d:
##                        pool.get('ir.property').write(cr,uid,d, p)
##                    else:
##                        pool.get('ir.property').create(cr,uid,p)
            ...
            if lin.startswith('^PRO("'):
                ...
                d2 = pool.get('res.partner').search(cr,uid,[('ref','=',ref)])
                ...
                if acc_id:
                    ...
                    p['res_id']= res_id
                    p['company_id']= c_company_id
                    p['value']= 'account.account,%d' % acc_id[0]
                    p['fields_id']= pool.get('ir.model.fields').search(cr,uid,[('name','=','property_account_payable')])[0]
...
wizard_load_partner('carreras.load_partner')
