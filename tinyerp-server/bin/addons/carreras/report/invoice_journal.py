...
##############################################################################
"""
  Fa el llistat del diari de vendes 
"""
import pooler
import time
import locale
...
class invoice_journal(report_sxw.rml_parse):
    ...
    def lines(self):
        ...
        if 'state' in self.datas['form']:
            if self.datas['form']['state'] != 'all':
                s.append(('state','=',self.datas['form']['state']))
        else:
            s.append(('state','in',['open','paid']))
        #s.append(('type','in',['out_invoice','out_refund']))
        s.append(('type','=','out_invoice'))
...
report_sxw.report_sxw('report.account.invoice.journal', 'account.invoice', 'addons/carreras/report/invoice_journal.rml',parser=invoice_journal)
