...
class invoice_journal(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(invoice_journal, self).__init__(cr, uid, name, context)
        self.localcontext.update( {
            ...
            'get_tot_expenses': self.get_tot_expenses,
            'get_tot_discount': self.get_tot_discount,
    ...
    def preprocess(self, objects, data, ids):
        super(invoice_journal, self).preprocess(objects, data, ids)
    ...
    def get_discount(self,inv):
        ...
        for line in inv.invoice_line:
            if line.price_subtotal < 0 and line.name == "Descuento":
                dto -= line.price_subtotal
        self.discount += dto
        return self.numf(dto)

    def get_expenses(self,inv):
        gas = 0.0
        for line in inv.invoice_line:
            if line.name == "Gastos Financieros":
                gas += line.price_subtotal 
        self.expenses += gas
        return self.numf(gas)
    ...
    def get_tot_discount(self):
        return self.numf(self.discount)
    def get_tot_expenses(self):
        return self.numf(self.expenses)
    ...
    def lines(self):
        ...
        s = [('company_id','child_of',[c_id])]
        s.append(('date_invoice','>=',start))
        s.append(('date_invoice','<=',end))
        if 'state' in self.datas['form'] and self.datas['form']['state']:
            if self.datas['form']['state'] != 'all':
                s.append(('state','=',self.datas['form']['state']))
        else:
            s.append(('state','in',['open','paid']))
        #s.append(('type','in',['out_refund']))
        s.append(('type','in',self.datas['form']['type']))
        if 'sequence_id' in self.datas['form'] and self.datas['form']['sequence_id']:
            s.append(('sequence_id','=',self.datas['form']['sequence_id']))
        if 'order' in self.datas['form']:
            order=self.datas['form']['order']
        else:
            order='number'
        ids = i_obj.search(self.cr,self.uid,s,order=order)
        ...
        # ordre per compte comptable
        """
        res=i_obj.browse(self.cr,self.uid,ids)
        res.sort(cmp=lambda x,y: cmp(x.account_id.code,y.account_id.code))
        return res
        """
...
report_sxw.report_sxw('report.account.invoice_report', 'account.invoice', 'addons/carreras/report/invoice_report.rml',parser=invoice_journal)
report_sxw.report_sxw('report.account.in_invoice_report', 'account.invoice', 'addons/carreras/report/in_invoice_report.rml',parser=invoice_journal)
report_sxw.report_sxw('report.account.out_invoice_report', 'account.invoice', 'addons/carreras/report/out_invoice_report.rml',parser=invoice_journal)
