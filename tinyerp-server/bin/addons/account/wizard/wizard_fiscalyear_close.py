...
def _data_save(self, cr, uid, data, context):
	...
	if data['form']['report_new']:
		...
		company_ids= pool.get('res.company')._get_company_children(cr,uid,user.company_id.id)
		cr.execute('select id from account_account WHERE active and company_id in (%s)' \
			% ",".join(map(str, company_ids)))
