...
"""
  Printers
"""
class printers(osv.osv):
    _name = "printjob.printers"
...
class printjob_job(osv.osv):
    ...
    _columns = {
        ...
        'keep':fields.boolean('Keep the document'),
...
class report_spool(netsvc.Service):
    ...
    def report(self, db, uid, passwd, object, ids, datas=None, context=None):
        ...
        # si la impressora de l'usuari és la 'pdf' es força online
                if system_printer == 'pdf':
                    batch = False
            else:
        ...
        id = pool.get("printjob.job").create(cr,uid,job)
        cr.commit()
        cr.close()
        ...
        def go(id, uid, ids, datas, context, system_printer):
            ...
            (result_obj, format_obj) = obj.create(cr, uid, ids, datas, context)
            if type(format_obj) == list:
                result_list=result_obj
                format_list=format_obj
            else:
                result_list=[result_obj]
                format_list=[format_obj]
            ...
            #logger.notifyChannel("report", netsvc.LOG_INFO,len(result_list))
            logger.notifyChannel("report", netsvc.LOG_INFO,format_list)
            for result,format in zip(result_list,format_list):
                #logger.notifyChannel("report", netsvc.LOG_INFO,result)
                logger.notifyChannel("report", netsvc.LOG_INFO,format)
                if format != 'raw' or len(format_list) == 1:
                    job = {
                        'result':base64.encodestring(result),
                        'format':format,
                        'state': system_printer and 'done' or 'ready',
                    }
                if system_printer:
                    if len(format_list) == 1 or format=='raw':
                        self.print_direct(result,system_printer,format)
            pool.get("printjob.job").write(cr,uid,id,job)
            cr.commit()
            cr.close()
            return True
        thread.start_new_thread(go, (id, uid, ids, datas, context, batch and system_printer))
        return id
    ...
    def report_get(self, db, uid, passwd, report_id):
        ...
        if report['batch'] and not res['state']:
            return {'state':True,'result':False}
        return res

report_spool()

""" Afegir l'impressora d'agulles a la taula d'usuaris """
class users(osv.osv):
    _name = "res.users"
    _inherit = "res.users"

    _columns = {
        'raw_printer': fields.many2one("printjob.printers","Impressora d'agulles"),
    }
