...
print_fields = {
    'printer' : {'string':'Impressora', 'type':'many2one', 'relation':'printjob.printers','required':True},
}
...
class wizard_reprint(wizard.interface):
    ...
    def _get_default_printer(self, cr, uid, data, context):
        ...
        printer_ids = pool.get('printjob.printers').search(cr, uid,[('is_default','=',True)])
    ...
    def _print(self, cr, uid, data, context):
        pool= pooler.get_pool(cr.dbname)
        printer_obj = pool.get('printjob.printers')
