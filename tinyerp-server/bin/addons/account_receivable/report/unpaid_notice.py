...
##############################################################################
"""
 Avís de reclamació d'impagat
"""
import pooler
import time
from report import report_sxw

class unpaid_notice(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(unpaid_notice, self).__init__(cr, uid, name, context)
    ...
    def preprocess(self, objects, data, ids):
        super(unpaid_notice, self).preprocess(objects, data, ids)

    def company_address(self,rec):
        ...
        return {
            ...
            'zip_city_state': ("%s %s %s" % (fields['zip'] or '',fields['city'] or '',
                        fields['state_id'] and ("(%s)" % fields['state_id'][1]) or '')).decode('utf8')[0:45],
            }
    
    def partner_address(self,rec):
    ...
    def _parse(self, rml_dom, objects, data, header=False):
        ...
        for elem in self.dom.getElementsByTagName("blockTable"):
            if elem.hasAttribute("style") and elem.getAttribute("style") == "Peu":
                elem.setAttribute("rowHeights","12.5,12.5")
            if elem.hasAttribute("style") and elem.getAttribute("style") == "Pagament":
                elem.setAttribute("rowHeights","30,30,30")
        res = self.dom.documentElement.toxml('utf-8')
        return res
        
        
report_sxw.report_sxw('report.unpaid.notice', 'account.receivable', 'addons/account_receivable/report/unpaid_notice.rml',
    parser=unpaid_notice,header=False)
